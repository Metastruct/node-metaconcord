...
export class SlashCustomRoleCommand extends SlashCommand {
	...
	constructor(bot: DiscordBot, creator: SlashCreator) {
		...
		bot.discord.on("guildMemberRemove", async member => {
			const role = this.getCustomRole(member.id); // not sure if this works but let's see
	...
	async run(ctx: CommandContext): Promise<any> {
		...
		switch (cmd) {
			...
			case "add_emoji":
				return this.addRoleIcon(ctx, false);
			case "add_icon":
				return this.addRoleIcon(ctx, true);
			case "remove":
				return this.removeRole(ctx);
			case "remove_icon":
				return this.removeRoleIcon(ctx);
			default:
				return false; // ??? I guess
		}
	}
	getGuildMember(userId: string): Discord.GuildMember | undefined {
		const guild = this.bot.discord.guilds.cache.get(DiscordConfig.bot.primaryGuildId);
		return guild?.members.cache.get(userId);
	}

	getCustomRole(userId: string): Discord.Role | undefined {
		return this.getGuildMember(userId)?.getCustomRole;
	}
	...
	async removeRoleIcon(ctx: CommandContext): Promise<any> {
		...
		const role = this.getCustomRole(ctx.user.id);
		if (!role) {
			return EphemeralResponse("You don't have a custom role yet!");
		}
		if (!role.icon && !role.unicodeEmoji) {
			return EphemeralResponse("You don't have an icon set!");
		}
		await role.setIcon("");
	...
	async addRoleIcon(ctx: CommandContext, download: boolean): Promise<any> {
		...
		if (download) {
			return this.uploadIcon(ctx, role);
		}
		return this.addEmoji(ctx.options.add_emoji.emoji, role);
	}
	...
	async addEmoji(emoji: string, role: Discord.Role): Promise<any> {
		...
		if (unicode) {
			...
			return EphemeralResponse(`Set your role emoji to ${emoji}`);
		}
		if (custom) {
			const emoji = this.bot.discord.emojis.cache.get(custom[3]);
			if (!emoji) {
				return EphemeralResponse(`Couldn't get your emote for some reason 🤷‍♂️ ${custom[3]}`);
			}
	...
	async removeRole(ctx: CommandContext): Promise<any> {
		...
		const role = this.getCustomRole(ctx.user.id);
		const member = this.getGuildMember(ctx.user.id);
		if (role) {
	...
	async addRole(ctx: CommandContext): Promise<any> {
		...
		if (!targetRole) {
			...
			const existingRole = this.getCustomRole(ctx.user.id);
			if (existingRole) {
			...
			const boosterRole = this.bot.discord.guilds.cache
				.get(ctx.guildID)
				?.roles.cache.get(DiscordConfig.roles.serverbooster);
			...
			try {
				...
				console.error(ex);
				return EphemeralResponse("Something went wrong changing the color of the role :(");
			}
		}
