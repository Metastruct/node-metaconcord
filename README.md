...
Game servers read accounts with the gamebridge token in `X-Auth-Token`: `GET /accounts/staff?provider=steam|minecraft` (platform id, platform name, roles of every account with a role and a proven link there, new developers included) and `GET /accounts/steam/:steamId64` or `/accounts/minecraft/:uuid` (name, roles, linked platforms, 404 `no account` without a proven link). aowl and the Minecraft mod rank from the first on boot and from the second on join.
