...
Game servers read accounts with the gamebridge token in `X-Auth-Token`: `GET /accounts/staff` (steamid64, name, roles of every account with a role and a proven Steam link, new developers included) and `GET /accounts/steam/:steamId64` (name, roles, linked platforms, 404 without a proven Steam link). aowl ranks from the first on boot and from the second on join.
